<?php

/**
 * Reinfeld Theme Customizer
 *
 * @package Reinfeld
 */

/**
 * Add postMessage support for site title and description for the Theme Customizer.
 *
 * @param WP_Customize_Manager $wp_customize Theme Customizer object.
 */
function reinfeld_customize_register($wp_customize)
{
  $wp_customize->get_setting("blogname")->transport = "postMessage";
  $wp_customize->get_setting("blogdescription")->transport = "postMessage";

  if (isset($wp_customize->selective_refresh)) {
    $wp_customize->selective_refresh->add_partial("blogname", [
      "selector" => ".site-title a",
      "render_callback" => "reinfeld_customize_partial_blogname",
    ]);
    $wp_customize->selective_refresh->add_partial("blogdescription", [
      "selector" => ".site-description",
      "render_callback" => "reinfeld_customize_partial_blogdescription",
    ]);
  }

  // Theme Options
  $wp_customize->add_section("reinfeld_options", [
    "title" => __("Theme Options", "reinfeld"),
    "priority" => 120,
  ]);

  // Hide related posts
  $wp_customize->add_setting("reinfeld_hide_related_posts", [
    "capability" => "edit_theme_options",
    "sanitize_callback" => "reinfeld_sanitize_checkbox",
    "transport" => "postMessage",
  ]);

  $wp_customize->add_control("reinfeld_hide_related_posts", [
    "type" => "checkbox",
    "section" => "reinfeld_options",
    "label" => __("Hide Related Posts", "reinfeld"),
    "description" => __("Whether to hide the related posts section on single posts.", "reinfeld"),
  ]);

  // Fallback Post Thumbnail
  $wp_customize->add_setting("reinfeld_fallback_thumbnail", [
    "capability" => "edit_theme_options",
    "sanitize_callback" => "absint",
    "transport" => "postMessage",
  ]);

  $wp_customize->add_control(
    new WP_Customize_Media_Control($wp_customize, "reinfeld_fallback_thumbnail", [
      "label" => __("Fallback Post Thumbnail", "reinfeld"),
      "description" => __('Select a fallback image to use when posts don\'t have a featured image.', "reinfeld"),
      "section" => "reinfeld_options",
      "mime_type" => "image",
    ]),
  );

  // Social Media Section
  $wp_customize->add_section("reinfeld_social_media", [
    "title" => __("Social Media Links", "reinfeld"),
    "priority" => 130,
  ]);

  // GitHub URL
  $wp_customize->add_setting("reinfeld_github_url", [
    "default" => "",
    "sanitize_callback" => "esc_url_raw",
  ]);

  $wp_customize->add_control("reinfeld_github_url", [
    "label" => __("GitHub URL", "reinfeld"),
    "section" => "reinfeld_social_media",
    "type" => "url",
  ]);

  // Bluesky URL
  $wp_customize->add_setting("reinfeld_bluesky_url", [
    "default" => "",
    "sanitize_callback" => "esc_url_raw",
  ]);

  $wp_customize->add_control("reinfeld_bluesky_url", [
    "label" => __("Bluesky URL", "reinfeld"),
    "section" => "reinfeld_social_media",
    "type" => "url",
  ]);

  // Mastodon URL
  $wp_customize->add_setting("reinfeld_mastodon_url", [
    "default" => "",
    "sanitize_callback" => "esc_url_raw",
  ]);

  $wp_customize->add_control("reinfeld_mastodon_url", [
    "label" => __("Mastodon URL", "reinfeld"),
    "section" => "reinfeld_social_media",
    "type" => "url",
  ]);

  // About Section
  $wp_customize->add_section("reinfeld_about_section", [
    "title" => __("About Section", "reinfeld"),
    "priority" => 140,
  ]);

  // About Text
  $wp_customize->add_setting("reinfeld_about_text", [
    "default" => "",
    "sanitize_callback" => "wp_kses_post",
  ]);

  $wp_customize->add_control("reinfeld_about_text", [
    "label" => __("About Text", "reinfeld"),
    "section" => "reinfeld_about_section",
    "type" => "textarea",
  ]);

  // About Page URL
  $wp_customize->add_setting("reinfeld_about_page_url", [
    "default" => "/about",
    "sanitize_callback" => "esc_url_raw",
  ]);

  $wp_customize->add_control("reinfeld_about_page_url", [
    "label" => __("About Page URL", "reinfeld"),
    "section" => "reinfeld_about_section",
    "type" => "url",
  ]);

  // Copyright Section
  $wp_customize->add_section("reinfeld_copyright", [
    "title" => __("Copyright Settings", "reinfeld"),
    "priority" => 160,
  ]);

  // Start Year
  $wp_customize->add_setting("reinfeld_start_year", [
    "default" => "2024",
    "sanitize_callback" => "sanitize_text_field",
  ]);

  $wp_customize->add_control("reinfeld_start_year", [
    "label" => __("Start Year", "reinfeld"),
    "section" => "reinfeld_copyright",
    "type" => "text",
  ]);

  // Custom Copyright Text
  $wp_customize->add_setting("reinfeld_copyright_text", [
    "default" => "",
    "sanitize_callback" => "wp_kses_post",
  ]);

  $wp_customize->add_control("reinfeld_copyright_text", [
    "label" => __("Custom Copyright Text", "reinfeld"),
    "description" => __("Leave empty to use automatic copyright text", "reinfeld"),
    "section" => "reinfeld_copyright",
    "type" => "textarea",
  ]);

  // License URL
  $wp_customize->add_setting("reinfeld_license_url", [
    "default" => "#ccbysa",
    "sanitize_callback" => "esc_url_raw",
  ]);

  $wp_customize->add_control("reinfeld_license_url", [
    "label" => __("License URL", "reinfeld"),
    "section" => "reinfeld_copyright",
    "type" => "url",
  ]);

  // License Text
  $wp_customize->add_setting("reinfeld_license_text", [
    "default" => "CC-BY-SA",
    "sanitize_callback" => "sanitize_text_field",
  ]);

  $wp_customize->add_control("reinfeld_license_text", [
    "label" => __("License Text", "reinfeld"),
    "section" => "reinfeld_copyright",
    "type" => "text",
  ]);
}
add_action("customize_register", "reinfeld_customize_register");

/**
 * Render the site title for the selective refresh partial.
 *
 * @return void
 */
function reinfeld_customize_partial_blogname()
{
  bloginfo("name");
}

/**
 * Render the site tagline for the selective refresh partial.
 *
 * @return void
 */
function reinfeld_customize_partial_blogdescription()
{
  bloginfo("description");
}

/**
 * Sanitize checkbox values.
 */
function reinfeld_sanitize_checkbox($checked)
{
  return isset($checked) && true === $checked ? true : false;
}
